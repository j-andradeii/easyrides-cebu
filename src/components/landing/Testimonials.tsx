 ...
 *
 * Real reviews approved in /admin/reviews are passed in as `reviews` and take
 * precedence; the curated quotes below are the fallback until enough real ones
 * are published (plan §7A.2).
 */

import type { PublishedTestimonial } from '@/lib/crm/testimonials';

const ACCENTS = ['from-coral to-mango', 'from-palm to-palm-dark', 'from-mango to-coral'];
...
export function Testimonials({ reviews = [] }: { reviews?: PublishedTestimonial[] }) {
  // Real published reviews win; curated quotes fill any remaining slots.
  const cards =
    reviews.length > 0
